...
backup() {
    ...
    # Copy the folder to the target directory with pv for progress bar
    rclone copy "$source_folder" "$target_directory/$copy_with_date" | pv -lep -s $(du -sb "$source_folder" | awk '{print $1}')
